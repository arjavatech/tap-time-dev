...
function viewCurrentDateReport() {
  ...
  const employeeApiURL = `https://yrvi6y00u8.execute-api.us-west-2.amazonaws.com/dev/employee/getall/${cid}`;
...
async function updateDailyReportAPiData(emp_id, cid, date, type, checkin_snap, checkin_time, checkout_snap, checkout_time, time_worked) {
  ...
  var apiBaseUrl = `https://yrvi6y00u8.execute-api.us-west-2.amazonaws.com/dev/dailyreport/update/${emp_id}/${cid}/${checkin_time}`;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  AddEmployee.addEventListener('click', (event) => {
    ...
    if (isValid) {
      ...
      // Send Post request
      fetch('https://yrvi6y00u8.execute-api.us-west-2.amazonaws.com/dev/dailyreport/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
